...
def _resolve_root_reference(schema: dict[str, Any]) -> dict[str, Any]:
  """Resolves root level $ref references within a JSON schema.

  This handles the common case where a schema generated by tools like
  Pydantic contains a top-level ``$ref`` field pointing to a definition in
  ``$defs`` or ``definitions``. Only local references (beginning with ``#/``)
  are supported.

  Args:
    schema: The raw OpenAPI/JSON schema dictionary.

  Returns:
    The schema dictionary with the root reference resolved if possible.
  """
...
def _to_gemini_schema(openapi_schema: dict[str, Any]) -> Schema:
  ...
  openapi_schema = _resolve_root_reference(openapi_schema)
